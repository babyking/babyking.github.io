...
指针变量是变量,是变量就有类型,   整型的97和'a'在机器层看来没有区别,只是到了上层会区别对待,这就是类型,类型也可以说是面对数如何处理. 指针变量和一个整型变量的区别,就是对待指针变量,变量里的值是另外一个整型的地址,可以取这个地址里的值 ,当然不取也可以,那这个地址就也可当成一个整型来看,定义是一回事,具体你怎么用是另外一回事. 跟前面的 97和'a',一样,你非把'a'当成97的整数也是可以的,但别忘了,高级语言的抽象就是为了解放心智负担.  所以指针变量跟普通变量在数据本质上是没有区别的,有区别的是针对他们操作惯例  #指针变量跟普通变量的区别  #数据类型你怎么理解
